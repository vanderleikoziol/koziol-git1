...
for i in range(20):
    for j in range(17):
        ...
        if k > x:
            x = k
            z = {f'Linha:{i} Coluna:{j}': matriz[i][j], f'Linha:[i] Coluna:[j + 1]': matriz[i][j + 1], f'Linha:{i} Coluna:{j + 2}': matriz[i][j + 2],
                 f'Linha:{i} Coluna:{j + 3}': matriz[i][j + 3]}
for i in range(17):
    for j in range(17):
        ...
        if k > x:
            x = k
            z = {f'Linha:{i} Coluna:{j}': matriz[i][j], f'Linha:{i + 1} Coluna:{j + 1}': matriz[i + 1][j + 1],
                 f'Linha:{i + 2} Coluna:{j + 2}': matriz[i + 3][j + 3]}
for i in range(3, 20):
    for j in range(17):
        k = matriz[i][j] * matriz[i - 1][j + 1] * matriz[i - 2][j + 2] * matriz[i - 3][j + 3]
        y += 1
        if k > x:
            x = k
            z = {f'Linha:{i} Coluna:{j}': matriz[i][j], f'Linha:{i - 1} Coluna:{j + 1}': matriz[i - 1][j + 1],
                 f'Linha:{i - 2} Coluna:{j + 2}': matriz[i - 2][j + 2], f'Linha:{i - 3} Coluna:{j + 3}': matriz[i - 3][j + 3]}
print(f'O maior produto é => {x}\nResultado da multiplicação de => {z}\nO nº de cálculo realizado foi => {y}')
